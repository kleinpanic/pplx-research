...
For more info: https://github.com/kleinpanic/pplx-research
    ...
    # Scope
    parser.add_argument("--site", "-s", help="Constrain search to specific domain")
    parser.add_argument(
        "--exclude",
        "-e",
        action="append",
    ...
    parser.add_argument("--region", "-r", help="Country/region code (e.g., US, UK, JP)")
    parser.add_argument("--language", "-l", help="Language filter (ISO 639-1 code, e.g., en, de, ja)")
    ...
    # Output options
    parser.add_argument("--return-images", action="store_true", help="Include image results in response")
    parser.add_argument(
    ...
    parser.add_argument("--webhook", "-w", help="POST results to webhook URL on completion")
    parser.add_argument("--quiet", "-q", action="store_true", help="Suppress progress output (for piping)")
    parser.add_argument(
        "--auto",
        "-a",
        action="store_true",
