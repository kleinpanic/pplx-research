...
class PerplexitySDK:
    ...
    def _call(self, endpoint: str, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            resp = requests.post(f"{self.base_url}/{endpoint}", headers=headers, json=payload, timeout=120)

            # Quota fallback
            if resp.status_code == 401 and "quota" in resp.text.lower() and not self.use_openrouter:
                if not self.quiet_mode:
                    print(
                        "[yellow]Perplexity quota exceeded, falling back to OpenRouter...[/yellow]",
                        file=sys.stderr,
                    )
            ...
            return {
                "error": str(e),
                "status_code": (
                    getattr(e.response, "status_code", 500) if hasattr(e, "response") and e.response else 500
                ),
            }

    def chat(self, query: str, model: str = "sonar", messages: Optional[list[dict]] = None, **kwargs) -> dict[str, Any]:
        """
    ...
    def stream(self, query: str, model: str = "sonar", **kwargs) -> Generator[dict[str, Any], None, None]:
        """
