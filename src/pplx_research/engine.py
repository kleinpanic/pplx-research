...
class ResearchEngine:
    ...
    SOURCE_PREFIXES = {
        "academic": (
            ...
            "OR site:dl.acm.org OR site:ieeexplore.ieee.org"
        ),
        "news": (
            "site:news.google.com OR site:reuters.com OR site:apnews.com " "OR site:bbc.com OR site:techcrunch.com"
        ),
...
Output ONLY the questions, one per line, no numbering or bullets."""  # noqa: E501
...
{{"mode": "quick or deep or synthesis", "depth": 1-5, "sources": ["one or more: academic,news,docs,forums,code,all"], "time_range": null or "day" or "week" or "month" or "year", "reasoning": "brief explanation"}}  # noqa: E501
    ...
    def _deep_mode(self) -> dict[str, Any]:
        ...
        with Progress(
            ...
            for i in range(self.depth):
                ...
                content = result["choices"][0]["message"]["content"]
                self._parse_citations(result)
                all_content.append({"iteration": i + 1, "query": gap, "content": content})
    ...
    def _synthesis_mode(self) -> dict[str, Any]:
        ...
        with Progress(
            ...
            for src_type in source_types:
                ...
                content = result["choices"][0]["message"]["content"]
                self._parse_citations(result)
                perspectives.append({"source_type": src_type, "content": content})
...
{chr(10).join(f"--- ITERATION {c['iteration']}: {c['query']} ---{chr(10)}{c['content']}" for c in all_content)}  # noqa: E501

Output a well-structured markdown report."""  # noqa: E501
...
{chr(10).join(f"--- {p['source_type'].upper()} PERSPECTIVE ---{chr(10)}{p['content']}" for p in perspectives)}  # noqa: E501

Output a comprehensive markdown report organized by key themes."""  # noqa: E501
    ...
    def run(self) -> str:
        ...
        if self.auto:
            ...
            if not self.quiet:
                console.print(
                    f"[green]Auto-detected: {self.mode} mode, depth {self.depth}, "
                    f"sources: {','.join(self.sources)}[/green]"
                )
